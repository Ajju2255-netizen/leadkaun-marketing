...
- ✅ L — City data moat: 55 verified economic notes + **302 cities now carry verified business-district + local-economy data** (web-verified vs govt MSME/RIICO/MIDC/GIDC/SIPCOT/SAIL sources; low-confidence cities excluded to hold the zero-fabrication bar). Sibling-page overlap dropped ~92% → 74-80% (measured). **Engine built + run over 6 batches**: any city given `districts` auto-promotes its keyword+role leaves. Result: **indexable 8,596 → 20,102** (2.3×), every added page backed by real local facts. ~127 smaller candidate cities remain (pop <130k, thinner verifiable data) — future careful batches; user chose to KEEP THE GATE and grow with real data, not un-gate thin pages.
